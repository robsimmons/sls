;; Output of the automatic derivaiton from cbneed.sls
;; Re-formatted with a bit of alpha renaming, reordering and indentation.
...
apply/lam: ev_apply (rans (lam (\x. E x))) E2 X >-> {ev_eval (E (var X)) (s X) * cont (lam1 E2 X)}.
apply/let: ev_apply (rans (let X E1 A))    E2 Xs >-> {ev_apply (rans A) E2 Xs * cont (let2 X E1)}.
...
force/lam: ev_force X (rans (lam (\x. E x))) C Xs >-> All E': exp. recomp C (lam (\x. E x)) E' -> {ev_eval E' Xs * cont (lam2 X E)}.
force/let: ev_force X (rans (let Y E1 A))    C Xs >-> {ev_force X (rans A) C Xs * cont (let3 Y E1)}.
